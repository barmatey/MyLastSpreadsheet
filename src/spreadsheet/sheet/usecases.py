...
def delete_rows(sheet: Sheet, row_indexes: list[int], repo: SheetRepo = SheetRepoFake()):
    sindex_usecases.delete_rows(sheet, row_indexes)
    sheet.size = (sheet.size[0] - len(row_indexes), sheet.size[1])
    repo.update(sheet)
